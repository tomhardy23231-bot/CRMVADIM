...
import { Receipt, CheckCircle2, CircleDashed, TrendingUp, TrendingDown, Tag, Info } from 'lucide-react';
        ...
        <CardHeader className="pb-3 border-b border-gray-100 bg-gray-50/50">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div>
              ...
              <CardDescription className="text-xs text-gray-500 mt-1 max-w-xl">
                Нажмите на строку чтобы увидеть детали. Выберите статью в колонке справа, чтобы привязать платёж к Бюджету — тогда Факт посчитается автоматически.
              </CardDescription>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0 flex-wrap">
              {totalIncome > 0 && (
                <div className="flex items-center gap-1 bg-emerald-50 text-emerald-700 text-xs font-semibold px-2.5 py-1.5 rounded-lg border border-emerald-200">
                  <TrendingUp className="w-3.5 h-3.5" />
                  +{formatUAH(totalIncome)}
                </div>
              )}
              {totalExpense > 0 && (
                <div className="flex items-center gap-1 bg-red-50 text-red-700 text-xs font-semibold px-2.5 py-1.5 rounded-lg border border-red-200">
                  <TrendingDown className="w-3.5 h-3.5" />
                  −{formatUAH(totalExpense)}
                </div>
              )}
              {unassignedCount > 0 && (
                <div className="flex items-center gap-1.5 bg-amber-50 text-amber-700 text-xs font-semibold px-2.5 py-1.5 rounded-lg border border-amber-200 animate-pulse">
                  <CircleDashed className="w-3.5 h-3.5" />
                  Не привязано: {unassignedCount}
                </div>
              )}
              {assignedCount > 0 && (
                <div className="flex items-center gap-1.5 bg-emerald-50 text-emerald-700 text-xs font-semibold px-2.5 py-1.5 rounded-lg border border-emerald-200">
