    ...
    <Card className="bg-white shadow-sm hover:shadow-md transition-all duration-200">
      <CardHeader className="pb-3 px-3 lg:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-sm lg:text-base font-semibold">
            <CalendarRange className="w-4 h-4 lg:w-4.5 lg:h-4.5 text-gray-500" />
      ...
      <CardContent className="p-0">
        {!hasAnyData ? (
          ...
          <>
            {/* ===== МОБИЛЬНОЕ представление — вертикальные карточки по неделям ===== */}
              ...
              {projectWeeks.map((w) => {
                ...
                return (
                  <div key={w} className={cn(
                    ...
                    isNeg ? 'border-red-200 bg-red-50/20' : 'border-gray-100'
                  )}>
                    <div className="px-3 py-2 bg-gray-50 border-b border-gray-100">
                      <p className="text-xs font-semibold text-gray-700 whitespace-pre-wrap">{formatWeekStr(w, lang)}</p>
                    </div>
                    <div className="grid grid-cols-3 gap-px bg-gray-100">
                      <div className="bg-white p-2.5 text-center">
                        <TrendingUp className="w-3 h-3 text-emerald-500 mx-auto mb-0.5" />
                        <p className="text-[9px] text-gray-400 uppercase tracking-wide">{tr('income')}</p>
                        <p className="text-xs font-bold text-emerald-700">
                          {data.totalIncome[w] > 0 ? formatUAH(data.totalIncome[w]) : '—'}
                        </p>
                      </div>
                      <div className="bg-white p-2.5 text-center">
                        <TrendingDown className="w-3 h-3 text-red-500 mx-auto mb-0.5" />
                        <p className="text-[9px] text-gray-400 uppercase tracking-wide">{tr('expenses')}</p>
                        <p className="text-xs font-bold text-red-700">
                          {data.totalExpense[w] > 0 ? formatUAH(data.totalExpense[w]) : '—'}
                        </p>
                      </div>
                      <div className={cn('p-2.5 text-center', isNeg ? 'bg-red-50' : 'bg-emerald-50/50')}>
                        <AlertTriangle className={cn('w-3 h-3 mx-auto mb-0.5', isNeg ? 'text-red-500' : 'text-emerald-500')} />
                        <p className="text-[9px] text-gray-400 uppercase tracking-wide">{tr('balance')}</p>
                        <p className={cn('text-sm font-bold', isNeg ? 'text-red-700' : 'text-emerald-700')}>
                          {formatUAH(data.balance[w])}
                        </p>
                        {isNeg && (
                          <p className="text-[8px] font-semibold text-red-600 mt-0.5">{tr('cash_gap')}</p>
                        )}
                      </div>
                    </div>
                  </div>
