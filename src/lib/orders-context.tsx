...
export interface OrdersState {
  ...
  updateOrderArchivedStatus: (orderId: string, isArchived: boolean, newStatus?: OrderStatus) => Promise<void>;
...
export function OrdersProvider({ children }: { children: ReactNode }) {
  ...
  const updateOrderArchivedStatus = useCallback(async (orderId: string, isArchived: boolean, newStatus?: OrderStatus) => {
    setOrders((prev) => prev.map((o) => {
      if (o.id !== orderId) return o;
      const updates: any = { isArchived };
    ...
    try {
      ...
      if (!res.ok) throw new Error();
      toast.success(isArchived ? 'Заказ отправлен в архив' : 'Заказ возвращен в работу');
    } catch {
      toast.error('Ошибка изменения статуса архива');
      const fresh = await fetchSingleOrder(orderId);
      if (fresh) setOrders((prev) => prev.map((o) => o.id === orderId ? fresh : o));
    }
  }, []);
  ...
  const value: OrdersState = {
    ...
    updateBudgetItemDate, updateOrderDates, updateOrderArchivedStatus, toggleStageFlag,
